 ...
 * Check24 Scraper
 *
 * Strategy: Check24 uses SmartRecruiters as their ATS.
 *
 * List API:   GET https://api.smartrecruiters.com/v1/companies/Check24/postings?country=de&limit=100
 * Detail API: GET https://api.smartrecruiters.com/v1/companies/Check24/postings/{id}
 *
 * To use: set the site URL to:
 *   https://api.smartrecruiters.com/v1/companies/Check24/postings?country=de&limit=100
 */
...
export const Check24Scraper: ScraperStrategy = {
  name: "check24",
  ...
  async scrape(config: SiteConfig): Promise<ScrapedJob[]> {
    ...
    try {
      ...
      for (const job of rawJobs) {
        ...
        let rawHtml = "";
        let applyUrl = `https://jobs.smartrecruiters.com/Check24/${job.id}`;
      ...
      console.error("[Check24Scraper] Error:", err);
    }
